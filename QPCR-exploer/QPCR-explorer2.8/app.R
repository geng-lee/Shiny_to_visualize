...
ui <- navbarPage(
    ...
    tabPanel(
        ...
        fluidRow(column(6,
                        h4(tags$strong('A',style="color:black"),tags$strong('.Procedures and Materials',style="color:#eb5e0b")),
                        tags$img(src='qpcr1.PNG',width=800,height=350),
                        hr(style="border-color: #194350"),
                        
                        h4(tags$strong('C',style="color:black"),tags$strong('.Fomula of quantify expression',style="color:#eb5e0b")),
                        tags$img(src='fumula.PNG',width=800,height=200)),
                 column(6,
                        
                        h4(tags$strong('B',style="color:black"),tags$strong('.RT-PCR amplifing curve',style="color:#eb5e0b")),
    ...
    tabPanel(
      'QPCR rawdata choose',icon = icon('arrows-alt'),
      ...
      fileInput('data',
                ...
                accept = c("text/csv",
                           ...
                           ".csv")),
      h4('1、The raw data showed here:'),
      hr(style="border-color: #f96d15"),
      DT::dataTableOutput('raw_table'),
      
      h4('2、Select analysis data:'),
      # fluidRow(column(6,radioButtons('analysis_type','Analysis type',choices = c('separate','integrated'),selected = 'separate',inline = T)),
      #          column(6,radioGroupButtons('number_project','Number of project',choices = c(1,2,3,4,5),selected = 1,individual = T))),
      
      hr(style="border-color: #f96d15"),
      
      fluidRow(column(6,uiOutput('chose_sample_name')),
               column(6,uiOutput('chose_gene_name'))),
    ...
    tabPanel(
        ...
        sidebarLayout(
            sidebarPanel(
              ...
              # fluidRow(column(6,),
              #          column(6,)),
              checkboxInput('upload_summarised_data',tags$strong('Upload summarised data to plot'),value = FALSE),
              uiOutput('upload_summa_data'),
              
              hr(style="border-color: black"),
                fluidRow(column(6,selectInput('control_type','Control group type',choices = c('with-errorbar','without-errorbar'),selected = 'with-errorbar')),
                         column(6,radioButtons('rep_well_number','Gene well numbers',choices = c(2,3),inline = T,selected = 2))),
                fluidRow(column(6,
                ...
                tags$style(HTML(".js-irs-0 .irs-single, .js-irs-0 .irs-bar-edge, .js-irs-0 .irs-bar {background: #29bb89}")),
                sliderInput('alpha','Column alpha',min = 0,max = 1,value = 1,step = 0.05),
                fluidRow(column(6,
                ...
                fluidRow(column(6,
                                tags$style(HTML(".js-irs-1 .irs-single, .js-irs-1 .irs-bar-edge, .js-irs-1 .irs-bar {background: #28b5b5}")),
                                tags$style(HTML(".js-irs-2 .irs-single, .js-irs-2 .irs-bar-edge, .js-irs-2 .irs-bar {background: #28b5b5}")),
                         sliderInput('base_size','theme text size',min = 0,max = 30,value = 16)),
                         column(6,
                         sliderInput('ylab_size','Y title size',min = 0,max = 30,value = 18))),
                
                fluidRow(column(6,
                                numericInput('y_range','Y range',min = 0,max = 10000,value = 10,step = 0.1)),
                         column(6,
                                numericInput('minor_breaks','Y breaks',min = 0,max = 1000,value = 2,step = 0.1)),
                         ),
                ...
                textInput('main','Plot title',value = 'gene relative expression barplot'),
                hr(style="border-color: #194350"),
                ...
                column(4,numericInput('width',
                                      'Width',min = 1,max = 100,step = 1,value = 27))),
            ...
            mainPanel(
                tabsetPanel(
                    tabPanel(
                        ...
                        fluidRow(column(8,
                                        plotOutput('barplot',width = 750,height = 500)),
                                 ...
                                 column(4,
                                        ...
                                        selectInput('type_order','Type order choose',choices = c('gene'='Target.Name','sample'='Sample.Name'),selected = 'Sample.Name'),
                                        radioButtons('order_nubmber',h4('Order select'),choices = c('Individual','Both'),selected = 'Individual',inline = T),
                                        fluidRow(column(12,textAreaInput('colunm_order','Colunm orders',value = 'sample1,sample2(individual)\ngene1,gene2(both)')))
                                        ))),
                        ...
                        wellPanel(
                            ...
                            fluidRow(
                                ...
                                column(3,
                                       ...
                                       textInput('tick_width','Tick breaks',value = 'tick_width = c(1,1)')),
                                column(3,
                                       tags$style("#rel_heights {font-size:20px;height:35px;}"),
                                       textInput('rel_heights','Rel heights',value = 'rel_heights = c(0.25, 0, 0.1)'))
                            ),
                            hr(style="border-color: #8ab6d6"),
                            fluidRow(column(3,sliderInput('bottom','bottom',min = 0,max = 1000,value = 0,step = 10)),
                                     column(3,sliderInput('left','left',min = 0,max = 2000,value = 200,step = 10)),
                                     column(3,sliderInput('top','top',min = 0,max = 1000,value = 200,step = 10)),
                                     column(3,sliderInput('right','right',min = 0,max = 500,value = 0,step = 10)))
                    ...
                    tabPanel(
                        ...
                        fluidRow(column(6,
                                h4('1、The data selected from you raw data'),
                                hr(style="border-color: #f96d15"),
                                DT::dataTableOutput('selcted_table')),
                                column(6,
                                h4('2、This is dT data'),
                                hr(style="border-color: #f96d15"),
                    ...
                    tabPanel(
                        ...
                        wellPanel(
                          fluidRow(column(3,
                                          ...
                                          radioButtons('merge_type','Merge type',choices = c('mean','raw'),selected = 'mean',inline = T)),
                                   column(3,
                                          numericInput('rep_samp_number','Replicates sample numbers:',min = 1,max = 10,value = 3,step = 1)),
                                   column(3,
                    ...
                    tabPanel(
                      ...
                      hr(style="border-color: #f96d15"),
                      h3('3、Fast T-test VS control group:'),
                      hr(),
                      DT::dataTableOutput('fast_test_table'),
                      actionButton('fast_ttest_submit','submit',icon = icon('frog')),
                      downloadButton('fast_ttest_table','Download')
                    )
                )
            )
        )
...
server <- function(input, output) {
  ...
  # ------------------------------chose_sample_name chose_gene_name
  output$chose_sample_name <- renderUI({
    selectInput('chose_analysis_sn','Choose analysis sample name',
                multiple = T,
                choices = raw_tab()$Sample.Name,
                selected = NULL,
                )
  })
  
  output$chose_gene_name <- renderUI({
    selectInput('chose_analysis_gn','Choose analysis gene name',
                multiple = T,
                choices = raw_tab()$Target.Name,
                selected = NULL,
    )
  })
  
  # -------------chose table output
  chose_analysis_tab <- reactive({
    chose_ana_dat <- raw_tab()[which(raw_tab()$Sample.Name %in% c(input$chose_analysis_sn) & raw_tab()$Target.Name %in% c(input$chose_analysis_gn)),]
  ...
  # -------------------------------------------------------------------
    ...
    output$selcted_table <- DT::renderDataTable({
      chose_analysis_tab()[,c("Sample.Name","Target.Name","CT")]
    })
    # --------------------------------------------------------------------------
    ...
    qp_filter_tab <- reactive({
      ...
      qp_dat <- chose_analysis_tab()[,c("Sample.Name","Target.Name","CT")]
    ...
    output$express_table <- DT::renderDataTable({
        ...
        # ------------------------------------------control_type with-errorbar
        if(input$control_type == 'with-errorbar'){

          control_mean <- summarySE(control, measurevar="dt", groupvars=c("Sample.Name","Target.Name"))[,c(1,2,4)]
          
          
          if(input$rep_well_number ==2){
            control_mean2 <- rbind(control_mean,control_mean)
          }else if(input$rep_well_number ==3){
            control_mean2 <- rbind(control_mean,control_mean,control_mean)

          }else{}

          control_mean2 <- arrange(control_mean2,Sample.Name,Target.Name)
          treat <- arrange(treat,Sample.Name,Target.Name)
          
          treat$ddt <- treat$dt-control_mean2$dt
          treat$FC <- 2^-treat$ddt
          treat_clean <- treat[,c("Sample.Name","Target.Name",'FC')]
          treat_clean
        }else if(input$control_type == 'without-errorbar'){
    ...
    bar <- eventReactive(input$submit,{
        ...
        if(input$merge_rep == TRUE){
          ...
          if(input$merge_type == 'mean'){
            change_name_dat <- mer_rep()[,c(1,2,4)]
          }else{
            change_name_dat <- mer_rep()
          }
        ...
        # --------------------------------------------------------------------upload summarised data
        # input$upload_summarised_data up_suma_data
        if(input$upload_summarised_data == FALSE){
          
        }else{
          infile <- input$up_suma_data$datapath
              ...
              if(type=='csv')
                  summa <- read.csv(infile,header = T,sep = ',') else
                    summa <- read.csv(infile,header = T,sep = '\t')
              
              tgc <- summa
        }
        # ----------------------column orders colunm_order colunm_order_change order_nubmber Individual
       
        if(input$colunm_order_change == TRUE & input$order_nubmber == 'Individual'){
          if(input$type_order == 'Sample.Name'){
            ...
            tgc$Target.Name <- factor(tgc$Target.Name,levels = c(order[1:length(order)]))
          }
          
        }else if(input$colunm_order_change == TRUE & input$order_nubmber == 'Both'){
          tmp <- unlist(strsplit(input$colunm_order,split = '\n'))
          order1 <- unlist(strsplit(tmp[1],split = ','))
          order2 <- unlist(strsplit(tmp[2],split = ','))
          
          tgc$Sample.Name <- factor(tgc$Sample.Name,levels = c(order1[1:length(order1)]))
          tgc$Target.Name <- factor(tgc$Target.Name,levels = c(order2[1:length(order2)]))
        }else{}
   ...
   if(input$ygap == FALSE){
       ...
       p5 <- p4 +
         theme(
           ...
           plot.margin = margin(50, 100, 0, 00)
         )
       ...
       # p6 bottom left top right
       legend <- cowplot::get_legend(p5 + theme(legend.box.margin = margin(input$bottom, input$left, input$top, input$right)))
       plot_grid(p6, legend, rel_widths = c(4,0))
       
       # add.legend(plot = p4,
       #            margin = c(top=1,right=1,bottom=1,left=100))
   }
    ...
    output$upload_summa_data <- renderUI({
      if(input$upload_summarised_data == FALSE){
        
      }else{
        fileInput('up_suma_data',
                  h4('Upload summarised data'),
                  accept = c("text/csv",
                             "text/comma-separated-values,text/plain",
                             ".csv"))
      }
    ...
    mer_rep <- eventReactive(input$mer_submit,{
      ...
      if(input$rep_samp_number == 1){
        ...
        tgc
      }else if(input$rep_samp_number == 4){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer2 = unlist(strsplit(mer[2],split = '_'))
        mer3 = unlist(strsplit(mer[3],split = '_'))
        mer4 = unlist(strsplit(mer[4],split = '_'))
        
        two1 = unlist(strsplit(mer1[1],split = ','))
        two2 = unlist(strsplit(mer2[1],split = ','))
        two3 = unlist(strsplit(mer3[1],split = ','))
        two4 = unlist(strsplit(mer4[1],split = ','))
        
        tgc[which(tgc$Sample.Name %in% c(two1[1:length(two1)])),'Sample.Name'] <- mer1[2]
        tgc[which(tgc$Sample.Name %in% c(two2[1:length(two2)])),'Sample.Name'] <- mer2[2]
        tgc[which(tgc$Sample.Name %in% c(two3[1:length(two3)])),'Sample.Name'] <- mer3[2]
        tgc[which(tgc$Sample.Name %in% c(two4[1:length(two4)])),'Sample.Name'] <- mer4[2]
        tgc
      }else if(input$rep_samp_number == 5){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer3 = unlist(strsplit(mer[3],split = '_'))
        mer4 = unlist(strsplit(mer[4],split = '_'))
        mer5 = unlist(strsplit(mer[5],split = '_'))
        
        two1 = unlist(strsplit(mer1[1],split = ','))
        two2 = unlist(strsplit(mer2[1],split = ','))
        two3 = unlist(strsplit(mer3[1],split = ','))
        two4 = unlist(strsplit(mer4[1],split = ','))
        two5 = unlist(strsplit(mer5[1],split = ','))
        
        tgc[which(tgc$Sample.Name %in% c(two1[1:length(two1)])),'Sample.Name'] <- mer1[2]
        tgc[which(tgc$Sample.Name %in% c(two2[1:length(two2)])),'Sample.Name'] <- mer2[2]
        tgc[which(tgc$Sample.Name %in% c(two3[1:length(two3)])),'Sample.Name'] <- mer3[2]
        tgc[which(tgc$Sample.Name %in% c(two4[1:length(two4)])),'Sample.Name'] <- mer4[2]
        tgc[which(tgc$Sample.Name %in% c(two5[1:length(two5)])),'Sample.Name'] <- mer5[2]
        tgc
      }else if(input$rep_samp_number == 6){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer4 = unlist(strsplit(mer[4],split = '_'))
        mer5 = unlist(strsplit(mer[5],split = '_'))
        mer6 = unlist(strsplit(mer[6],split = '_'))
        ...
        two4 = unlist(strsplit(mer4[1],split = ','))
        two5 = unlist(strsplit(mer5[1],split = ','))
        two6 = unlist(strsplit(mer6[1],split = ','))
        ...
        tgc[which(tgc$Sample.Name %in% c(two4[1:length(two4)])),'Sample.Name'] <- mer4[2]
        tgc[which(tgc$Sample.Name %in% c(two5[1:length(two5)])),'Sample.Name'] <- mer5[2]
        tgc[which(tgc$Sample.Name %in% c(two6[1:length(two6)])),'Sample.Name'] <- mer6[2]
        tgc
      }else if(input$rep_samp_number == 7){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer5 = unlist(strsplit(mer[5],split = '_'))
        mer6 = unlist(strsplit(mer[6],split = '_'))
        mer7 = unlist(strsplit(mer[7],split = '_'))
        ...
        two5 = unlist(strsplit(mer5[1],split = ','))
        two6 = unlist(strsplit(mer6[1],split = ','))
        two7 = unlist(strsplit(mer7[1],split = ','))
        ...
        tgc[which(tgc$Sample.Name %in% c(two5[1:length(two5)])),'Sample.Name'] <- mer5[2]
        tgc[which(tgc$Sample.Name %in% c(two6[1:length(two6)])),'Sample.Name'] <- mer6[2]
        tgc[which(tgc$Sample.Name %in% c(two7[1:length(two7)])),'Sample.Name'] <- mer7[2]
        tgc
      }else if(input$rep_samp_number == 8){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer6 = unlist(strsplit(mer[6],split = '_'))
        mer7 = unlist(strsplit(mer[7],split = '_'))
        mer8 = unlist(strsplit(mer[8],split = '_'))
        ...
        two6 = unlist(strsplit(mer6[1],split = ','))
        two7 = unlist(strsplit(mer7[1],split = ','))
        two8 = unlist(strsplit(mer8[1],split = ','))
        ...
        tgc[which(tgc$Sample.Name %in% c(two6[1:length(two6)])),'Sample.Name'] <- mer6[2]
        tgc[which(tgc$Sample.Name %in% c(two7[1:length(two7)])),'Sample.Name'] <- mer7[2]
        tgc[which(tgc$Sample.Name %in% c(two8[1:length(two8)])),'Sample.Name'] <- mer8[2]
        tgc
      }else if(input$rep_samp_number == 9){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer7 = unlist(strsplit(mer[7],split = '_'))
        mer8 = unlist(strsplit(mer[8],split = '_'))
        mer9 = unlist(strsplit(mer[9],split = '_'))
        ...
        two7 = unlist(strsplit(mer7[1],split = ','))
        two8 = unlist(strsplit(mer8[1],split = ','))
        two9 = unlist(strsplit(mer9[1],split = ','))
        ...
        tgc[which(tgc$Sample.Name %in% c(two7[1:length(two7)])),'Sample.Name'] <- mer7[2]
        tgc[which(tgc$Sample.Name %in% c(two8[1:length(two8)])),'Sample.Name'] <- mer8[2]
        tgc[which(tgc$Sample.Name %in% c(two9[1:length(two9)])),'Sample.Name'] <- mer9[2]
        tgc
      }else if(input$rep_samp_number == 10){
        repn = unlist(strsplit(input$change_name,split = '\n'))
        mer <- c(repn[1:length(repn)])
        ...
        mer8 = unlist(strsplit(mer[8],split = '_'))
        mer9 = unlist(strsplit(mer[9],split = '_'))
        mer10 = unlist(strsplit(mer[10],split = '_'))
        ...
        two8 = unlist(strsplit(mer8[1],split = ','))
        two9 = unlist(strsplit(mer9[1],split = ','))
        two10 = unlist(strsplit(mer10[1],split = ','))
        ...
        tgc[which(tgc$Sample.Name %in% c(two8[1:length(two8)])),'Sample.Name'] <- mer8[2]
        tgc[which(tgc$Sample.Name %in% c(two9[1:length(two9)])),'Sample.Name'] <- mer9[2]
        tgc[which(tgc$Sample.Name %in% c(two10[1:length(two10)])),'Sample.Name'] <- mer10[2]
        tgc
      }
    ...
    output$change_name_table <- renderDataTable({
      if(input$merge_type == 'mean'){
        ...
        datatable(mer_rep(),options = list(pageLength = 5))
        }
    ...
    output$changename_table <- downloadHandler(
      ...
      content = function(file) {
        if(input$merge_type == 'mean'){
          ...
          write.csv(mer_rep(),file,row.names = F)
          }
    ...
    # ------------------------------------------------------------------------------fast t.test
    # fast_test_table fast_ttest_submit fast_ttest_table
    fast_test_res <- eventReactive(input$fast_ttest_submit,{
      
      if(input$merge_rep == FALSE){
        ...
        pval <- compare_means(FC~Sample.Name,data = treat_clean,
                      method = 't.test',
                      paired = as.logical(input$paired),
                      group.by = 'Target.Name',
                      ref.group = input$contrl_group,
                      var.equal=as.logical(input$var.equal),
                      p.adjust.method = input$p.adjust.methods,
        )
        pval
        ...
        if(input$merge_type == 'mean'){
          mer_rep_expression <- mer_rep()[,c(1,2,4)]
        }else{
          mer_rep_expression <- mer_rep()
        }
        
        
        pval <- compare_means(FC~Sample.Name,data = mer_rep_expression,
                              method = 't.test',
                              paired = as.logical(input$paired),
                              group.by = 'Target.Name',
                              ref.group = input$control_group_when_mer,
                              var.equal=as.logical(input$var.equal),
                              p.adjust.method = input$p.adjust.methods,
        )
        pval
    ...
    output$fast_test_table <- renderDT({
      fast_test_res()
    })
    
    output$fast_ttest_table <- downloadHandler(
      filename = function() {
        paste('T.test_results_data','csv', sep = '.')
      },
      
      content = function(file) {
        
        write.csv(fast_test_res(),file,row.names = F)
      })
}
