...
sidebar <- dashboardSidebar(
    sidebarMenu(
        menuItem('Introduction',tabName = 'Introduction',icon = icon('book-reader')),
        h4('1.GO Enrichment'),
...
body <- dashboardBody(
    ...
    tabItems(
        ...
        tabItem(tabName = 'Introduction',
                h2(tags$strong("GO and GSEA Enrichment Analysis and Visualization"),align='left'),
                h4(div('We developed this application to do GO , KEGG and GSEA Enrichment , besides we also can download the Enrichment results and graphs here. 
                        You can also offer you own GO results to plot individualy . If
                       you have any advice or suggestions , please connect with : 3219030654@stu.cpu.edu.cn ! Thank you for your support .')),
                br(),
                h4(tags$p('The analysis procedures are mainly done by clusterProfiler R package with',tags$strong('"enrichGO"') , tags$strong('"enrichkegg"') , tags$strong('"gseGO"'), 'and',tags$strong( '"gseKEGG"') ,'.')),
                h4('The format data you upload shows here:',style="color:orange"),
                
                fluidRow(box(title = 'genelist format',solidHeader = F,collapsible = F,status="success",width = 4,
                    h4(tags$img(src='genelist.PNG',width=175,height=250),align='center')),
                
                box(title = 'FC genelist format',solidHeader = F,collapsible = F,status="success",width = 4,
                    h4(tags$img(src='fc_genelist.PNG',width=250,height=250),align='center')),
                
                box(title = 'Barplot data format',solidHeader = F,collapsible = F,status="success",width = 4,
                    h4(tags$img(src='barplot.PNG',width=350,height=250),align='center')),
                
                box(title = 'Dotplot data format',solidHeader = F,collapsible = F,status="success",width = 6,
                    h4('Note: This data is from', tags$a(href='http://kobas.cbi.pku.edu.cn/kobas3/genelist/','KABAS'), 'web analysis , the column "Enrich_ratio" should have the same name .'),
                    hr(),
                    h4(tags$img(src='dotplot.PNG',width=550,height=250),align='center')),
                ))
    )
)
...
server <- function(input, output) {
    ...
    go_sim_res <- eventReactive(input$go_submit,{
        ...
        simp_data<- clusterProfiler::simplify(go_res(),
                                                 cutoff = 0.7,
                                                 by = "p.adjust",
                                                 select_fun = min,
                                                 measure = "Wang")
        sim <- data.frame(simp_data)
    })
